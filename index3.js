...
fetchPokemons().then(pokemons => {
    ...
    pokemons.results.forEach(pokemon => {
        ...
        pokemonDiv.setAttribute("data-name", pokemon.name); // Store pokemon name as data attribute
        pokemonDiv.innerHTML = `${pokemon.name}`;
        pokemonDiv.style.cursor = "pointer";

        // Fetching additional details including image URL
    ...
    // Styling individual pokemon div containers
    const pokemonDivs = document.querySelectorAll(".pokemon");
    pokemonDivs.forEach(pokemonDiv => {
